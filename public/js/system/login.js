...
socket.on('loginStatus', (data) => {
    if (data.status) {
        if (data.userStatus) {
            ...
            $(".login-progressbar-indicator h3").html('0%');
            neonLogin.$login_progressbar.width(0);
            toastr['error']('Hesabınız pasif duruma alındığı için giriş işlemi gerçekleşmedi!');
        };
    }
    else {
        //Login false
        ...
        $(".login-progressbar-indicator h3").html('0%');
        neonLogin.$login_progressbar.width(0);
        toastr['error']('Giriş bilgileriniz hatalı!');
    };
